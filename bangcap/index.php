...
$truyvanMaBC = "
SELECT
CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))=1 
THEN 
	CONCAT('BC000',CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))
ELSE 
	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))=2
    THEN
    	CONCAT('BC00',CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))
    ELSE
    	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))=3
    	THEN
    		CONCAT('BC0',CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))
    	ELSE
    		CONCAT('BC',CONVERT(CONVERT(RIGHT(MaBC,4),int)+1,CHAR))
    	END
    END
...
<body>
	...
	<div class="col-md-9 ms-sm-auto col-lg-10"> 
		<!-- Webview -->
			...
			<div class="card">
					<div class="card-header">
						<center><h3><b>THÔNG TIN MÃ BẰNG CẤP</b></h3></center>
					</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function LoadData(){
				$.ajax({
					url: "http://localhost:8080/QLNS/bangcap/ajax_xuly.php",
					method: "POST",
			...
			$(document).on('click','.edit_data',function(){
				...
				$.ajax({
					url: "http://localhost:8080/QLNS/bangcap/edit.php",
					method: "POST",
					dataType: "json",
					data:{MaBC:MaBC},
					success:function(data){
			...
			$('#edit_bangcap').on('submit',function(event){
				...
				if ($('#MaBC').val()=='') {
					...
					$.ajax({
						url: "http://localhost:8080/QLNS/bangcap/ajax_sua.php",
						method: "POST",
