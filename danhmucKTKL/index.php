...
$truyvanMaDM = "
SELECT
CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))=1 
THEN 
	CONCAT('KK000',CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))
ELSE 
	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))=2
    THEN
    	CONCAT('KK00',CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))
    ELSE
    	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))=3
    	THEN
    		CONCAT('KK0',CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))
    	ELSE
    		CONCAT('KK',CONVERT(CONVERT(RIGHT(MaKTKL,4),int)+1,CHAR))
    	END
    END
...
<body>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function LoadData(){
				$.ajax({
					url: "http://localhost:8080/QLNS/danhmucKTKL/ajax_xuly.php",
					method: "POST",
			...
			$(document).on('click','.edit_data',function(){
				...
				$.ajax({
					url: "http://localhost:8080/QLNS/danhmucKTKL/edit.php",
					method: "POST",
					dataType: "json",
					data:{MaKTKL:MaKTKL},
					success:function(data){
			...
			$('#edit_DM').on('submit',function(event){
				...
				if ($('#MaKTKL').val()=='') {
					...
					$.ajax({
						url: "http://localhost:8080/QLNS/danhmucKTKL/ajax_sua.php",
						method: "POST",
