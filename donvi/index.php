...
$truyvanMaDV = "
SELECT
CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))=1 
THEN 
	CONCAT('DV000',CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))
ELSE 
	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))=2
    THEN
    	CONCAT('DV00',CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))
    ELSE
    	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))=3
    	THEN
    		CONCAT('DV0',CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))
    	ELSE
    		CONCAT('DV',CONVERT(CONVERT(RIGHT(MaDV,4),UNSIGNED)+1,CHAR))
    	END
    END
...
<body>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function LoadData(){
				$.ajax({
					url: "<?php echo $HomeURL; ?>/donvi/ajax_xuly.php",
					method: "POST",
			...
			$(document).on('click','.edit_data',function(){
				...
				$.ajax({
					url: "<?php echo $HomeURL; ?>/donvi/edit.php",
					method: "POST",
					dataType: "json",
					data:{MaDV:MaDV},
					success:function(data){
			...
			$('#edit_donvi').on('submit',function(event){
				...
				if ($('#MaDV').val()=='') {
					...
					$.ajax({
						url: "<?php echo $HomeURL; ?>/donvi/ajax_sua.php",
						method: "POST",
