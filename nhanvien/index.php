...
$truyvanMaNV = "
SELECT
CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))=1 
THEN 
	CONCAT('NV000',CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))
ELSE 
	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))=2
    THEN
    	CONCAT('NV00',CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))
    ELSE
    	CASE WHEN CHAR_LENGTH(CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))=3
    	THEN
    		CONCAT('NV0',CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))
    	ELSE
    		CONCAT('NV',CONVERT(CONVERT(RIGHT(MaNV,4),int)+1,CHAR))
    	END
    END
...
<!--<script type="text/javascript">
  function myFunction() {
  location.replace("http://localhost:8080/QLNS/nhanvien/index.php")
}-->
</script>
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function LoadData(){
				$.ajax({
					url: "http://localhost:8080/QLNS/nhanvien/ajax_xuly.php",
					method: "POST",
			...
			$(document).on('click','.edit_data',function(){
				...
				$.ajax({
					url: "http://localhost:8080/QLNS/nhanvien/edit.php",
					method: "POST",
					dataType: "json",
					data:{MaNV:MaNV},
					success:function(data){
			...
			$('#edit_nhanvien').on('submit',function(event){
				...
				if ($('#MaNV').val()=='') {
					...
					$.ajax({
						url: "http://localhost:8080/QLNS/nhanvien/ajax_sua.php",
						method: "POST",
